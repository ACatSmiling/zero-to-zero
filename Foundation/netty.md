...
>
>**$\color{RubineRed}{Since: 2024-10-05}$**

## NIO

**`NIO`：Non-Blocking IO，即非阻塞 IO。**

### 三大组件

#### Channel（通道）

**`Channel`**：**通道，是一种用于在字节缓冲区和外部数据源（如文件、套接字等）之间进行数据传输的连接。**它类似于传统 I/O 中的流，但又有所不同。通道可以进行双向的数据传输，既可以用于读取数据，也可以用于写入数据。

常见的 Channel 有：

* `FileChannel`
  * **主要用于文件的读写操作。**它可以从文件中读取数据到缓冲区，也可以将缓冲区中的数据写入文件。
  * 例如，在文件复制操作中，可以使用 FileChannel 来高效地将一个文件的内容复制到另一个文件。
* `DatagramChannel`
  * **用于基于 UDP 协议的网络通信的通道。**它提供了一种无连接的、基于数据报的通信方式。
  * 例如，在实时视频流传输系统中，每一个视频帧可以看作是一个 UDP 数据报进行发送。由于视频数据对实时性要求很高，偶尔丢失一两个数据报（帧）可能不会对观看体验造成严重影响，而且 UDP 的低延迟特性可以更好地满足实时性要求。DatagramChannel 可以高效地将这些视频数据报发送到接收端。
* `SocketChannel`
  * **用于基于 TCP 协议的网络通信中的套接字读写操作。**它可以连接到远程服务器的套接字，进行数据的发送和接收。
  * 例如，在客户端—服务器通信场景中，客户端可以使用 SocketChannel 连接到服务器，并发送请求数据。
* `ServerSocketChannel`
...
**`Buffer`**：**缓冲区，是一块用于存储数据的内存区域。**在 NIO 中，数据的读写操作都是通过缓冲区来进行的。它提供了一种高效的方式来处理数据，避免了频繁地直接从数据源读取或写入数据，从而提高了 I/O 操作的效率。
...
* ShortBuffer、IntBuffer、LongBuffer、FloatBuffer、DoubleBuffer
  * 这些缓冲区用于存储基本数据类型，如整数、长整数等。在处理需要存储和传输这些基本数据类型的场景中非常有用，例如，在网络通信中传输整数数组等数据。

#### Selector（选择器）

**`Selector`**：**选择器，是 NIO 中用于实现单线程处理多个通道的 I/O 事件的关键组件。**它可以同时监听多个通道的事件（如读事件、写事件、连接事件等），当某个通道有感兴趣的事件发生时，选择器会通知应用程序，这样就可以在一个线程中高效地处理多个通道的 I/O 操作，避免了为每个通道创建一个线程的资源浪费。

单从字面意思不好理解 Selector，下面结合服务器的设计演化来理解它的用途：
