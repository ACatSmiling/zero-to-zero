...
- `标志位 (Flags)`
  ...
  - **`SYN (Synchronization)`**
    ...
    ![image-20230202183616869](network-protocol/image-20230202183616869.png)

    - 当 SYN = 1、ACK = 0 时，表明这是以恶搞建立连接的请求。
...
- A 收不到 B 的确认信息，等待超时后，重新发送 M1 给 B。
...
![image-20230204214238347](network-protocol/image-20230204214238347.png)

使用连续 ARQ 协议，加滑动窗口协议，数据传输过程如下：

![image-20230204233710213](network-protocol/image-20230204233710213.png)

> 窗口的大小，不是固定的，B 返回的确认信息中，包含下次 A 发送数据的窗口大小。

###### SACK

TCP 通信过程中，如果发送摩纳哥序列中间某个数据包丢失（例如 1、2、3、4、5 中的 3 丢失了），TCP 会通过重传最后确认的分组后续的分组（最后确认的是 2，会重传 3、4、5），这样，原先已经正确传输的分组，也可能重复发送（4、5），这降低了 TCP 的性能。
...
- 一对边界信息需要占用 8 个字节，由于 TCP 首部的选项部分最多 40 字节，所以：
  ...
  ![image-20230206153537984](network-protocol/image-20230206153537984.png)
